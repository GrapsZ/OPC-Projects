...
<section class="tc-thumbnail entry-media__holder <?php czr_fn_echo( 'element_class' ) ?>" <?php czr_fn_echo('element_attributes') ?>>
  ...
  if ( czr_fn_get_property( 'media_template' ) ):
    if ( czr_fn_get_property( 'has_permalink' ) ) : ?>
      <a class="<?php czr_fn_echo( 'link_class' ) ?>" rel="bookmark" title="<?php the_title_attribute( array( 'before' => __('Lien permanent vers:&nbsp;', 'strasbourg') ) ) ?>" href="<?php the_permalink() ?>"></a>
  <?php
    endif; //bg-link
  ...
  ?>
      <div class="post-type__icon">
        <a class="bg-icon-link icn-format" rel="bookmark" title="<?php the_title_attribute( array( 'before' => __('Lien permanent vers ', 'strasbourg') ) ) ?>" href="<?php the_permalink() ?>"></a>
      </div>
