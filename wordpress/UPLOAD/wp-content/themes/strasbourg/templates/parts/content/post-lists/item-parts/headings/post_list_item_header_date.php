...
<header class="entry-header <?php czr_fn_echo( 'element_class' ) ?>" <?php czr_fn_echo('element_attributes') ?>>
  <div class="entry-header-inner <?php czr_fn_echo( 'entry_title_class' ) ?>">
    ...
    <h2 class="entry-title ">
      <a class="czr-title" href="<?php the_permalink() ?>" title="<?php the_title_attribute( array( 'before' => __('Lien permanent vers ', 'strasbourg') ) ) ?>" rel="bookmark"><?php czr_fn_echo( 'the_title' ) ?></a>
    </h2>
    <?php
