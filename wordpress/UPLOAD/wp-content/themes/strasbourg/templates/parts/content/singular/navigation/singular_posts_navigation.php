  ...
  <nav id="nav-below" class="col-12" role="navigation">
    ...
    <ul class="czr_pager row flex-row flex-no-wrap">
      ...
      <?php if ( null != $prev_link ) : ?>
        <span class="sr-only"><?php _e('Article précédent', 'strasbourg') ?></span>
        <span class="nav-previous nav-dir"><?php echo $prev_link ?></span>
      <?php endif; ?>
      </li>
      <li class="nav-back col-2 text-center">
        <?php if ( is_single() ) : ?>
        <a href="<?php echo esc_url( ( 'page' != get_option( 'show_on_front' ) || ! get_option( 'page_for_posts' ) ) ? home_url( '/' ) : get_permalink( get_option( 'page_for_posts' ) ) ) ?>" title="<?php _e( 'Retour à la liste des articles', 'strasbourg') ?>">
          <span><i class="icn-grid-empty"></i></span>
          <span class="sr-only"><?php _e( 'Retour à la liste des articles', 'strasbourg') ?></span>
        </a>
        <?php endif ?>
      </li>
      <li class="next col-5 <?php echo $tnext_align_class ?>">
      <?php if ( null != $next_link ) : ?>
        <span class="sr-only"><?php _e('Article suivant', 'strasbourg') ?></span>
