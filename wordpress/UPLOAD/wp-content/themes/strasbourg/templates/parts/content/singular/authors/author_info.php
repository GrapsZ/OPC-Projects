  ...
  <?php foreach ( $authors_id as $author_id ) :?>
    <figure class="author-info">
      ...
      <figcaption>
        ...
        <a href="<?php echo esc_url( get_author_posts_url( $author_id ) ) ?>" rel="author" class="action-link" title="<?php _e( 'Voir tous les articles de l\'auteur', 'strasbourg' ) ?>">
          <?php
            $author_posts = count_user_posts( $author_id );
            printf( _n('%s article', '%s articles', $author_posts , 'strasbourg' ), $author_posts );
          ?>
        </a>
